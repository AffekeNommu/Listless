...
<body>
  <div class="container">
    <div class="row">
      ...
      <button type="button" class="btn btn-success" onclick="loadCategories()" data-toggle="modal" data-target="#newItemModal" rel="tooltip" data-placement="bottom" title="add item" data-animation="true"><i class="material-icons">add</i></button>
      <button type="button" class="btn btn-success" onclick="removeTicked()" rel="tooltip" data-placement="bottom" title="remove ticked" data-animation="true"><i class="material-icons">delete_sweep</i></button>
      <button type="button" class="btn btn-success" onclick="undoRemoved()" rel="tooltip" data-placement="bottom" title="undo remove" data-animation="true"><i class="material-icons">undo</i></button>
    ...
    <button type="button" class="btn btn-success" data-toggle="modal" data-target="#newCategoryModal" rel="tooltip" data-placement="bottom" title="add category" data-animation="true"><i class="material-icons">speaker_notes</i></button>
    <button type="button" class="btn btn-success" onclick="selRemoveCategory()" data-toggle="modal" data-target="#delCategoryModal" rel="tooltip" data-placement="bottom" title="delete category" data-animation="true"><i class="material-icons">speaker_notes_off</i></button>
  ...
  <div class="container">
    <div class="row">
      <div class="col-6">
        <br>
        <label for="inputItem">List ID</label>
        <input type="listname" class="form-control" id="inputList" aria-describedby="categoryInput" placeholder="List Name">
        <button type="button" class="btn btn-success" onclick="addList()" rel="tooltip" data-placement="bottom" title="Add List" data-animation="true"><i class="material-icons">add</i></button>
        <div id="expiry"></div>
      </div>
      <div class="col-6"></div>
    </div>
    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div class="modal-footer">
          <button type="button" onclick="newItem()" class="btn btn-success" rel="tooltip" data-placement="bottom" title="save changes" data-animation="true"><i class="material-icons">save</i></button>
        </div>
      </div>
    </div>
  </div>
  ...
  <div class="modal fade" id="editItemModal" tabindex="-1" role="dialog" aria-labelledby="editItemModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div class="modal-footer">
          <button type="button" onclick="editItem()" class="btn btn-success" rel="tooltip" data-placement="bottom" title="save changes" data-animation="true"><i class="material-icons">save</i></button>
        </div>
      </div>
    </div>
  </div>
  ...
  <div class="modal fade" id="delCategoryModal" tabindex="-1" role="dialog" aria-labelledby="delCategoryModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div class="modal-footer">
          <button type="button" onclick="removeCategory()" class="btn btn-success" rel="tooltip" data-placement="bottom" title="remove category" data-animation="true"><i class="material-icons">save</i></button>
        </div>
      </div>
    </div>
  </div>
  ...
  <div class="modal fade" id="newCategoryModal" tabindex="-1" role="dialog" aria-labelledby="newCategoryModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div class="modal-footer">
          <button type="button" onclick="newCategory()" class="btn btn-success" rel="tooltip" data-placement="bottom" title="add category"><i class="material-icons">save</i></button>
        </div>
      </div>
    </div>
  </div>
...
<script>
  ...
  function addList() {
    //Cookies.set('listlessID',$('#inputList').val(), { expires: 14 });
    toastr.success("Your list with ID: " + listID + " is set");
    ...
    Cookies.set('listlessID', JSON.stringify(data), {
      expires: 14
    });
    // Retrieve and parse the JSON object from the cookie
    const cookie = Cookies.get('listlessID');
    if (cookie) {
      const parsedCookie = JSON.parse(cookie);
      $('#expiry').text('expires on: ' + parsedCookie.expires);
    }

  }

  function loadList() {
    if (Cookies.get('listlessID') == undefined) {
      //no cookie set so set it to the default
      toastr.warning("No list name set");
    } else {
      //we have a cookie so set the input field to it
      const cookie = Cookies.get('listlessID');
      if (cookie) {
        const parsedCookie = JSON.parse(cookie);
        console.log('Value:', parsedCookie.value);
        console.log('Expires on:', parsedCookie.expires);
        $('#inputList').val(parsedCookie.value);
        listID = parsedCookie.value;
        $('#expiry').text('expires on: ' + parsedCookie.expires);
      }
    }
  }
  $(document).ready(function() {
    ...
    //Load the list name from cookie if it exists
    loadList();
